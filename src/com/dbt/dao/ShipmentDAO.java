	...
	public List<Shipment> getShipment(int orderID) {
		...
		try {
			...
			while (res.next()) {
				...
				PreparedStatement ps = con.prepareStatement("select _id,category,name,order_item.quantity,amount from order_item join product on order_item.product_id = product._id where ship_id = ?");
				ps.setInt(1, shipment.getId());
				ResultSet set = ps.executeQuery();
				List<Product> products = new ArrayList<>();
				while(set.next())
				{
					products.add(new Product(set.getInt("_id"), set.getInt("category"), set.getString("name"), set.getInt("quantity"), set.getInt("amount"), 0));
				}
