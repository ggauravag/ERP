    ...
    %>
         ...
         <td><input type="checkbox" name="kk"></td>
                <td style="width: 300px;text-align: left;background-color:#f5f5f5">
                    ...
                    <a href="<%=request.getContextPath() %>/dba/tabledata.jsp?server=<%=server%>&db=<%=db%>&table=<%=localtable%>" target="jspmain"><%=localtable%></a></td>
                <td style="width: 400px;text-align: left;background-color:#f5f5f5">
                	<center>
                	
                	<a href="<%=request.getContextPath() %>/dba/tabledata.jsp?server=<%=server%>&db=<%=db%>&table=<%=localtable%>" target="jspmain"><img class="icon" src="<%=request.getContextPath() %>/dba/Images/browse.png" alt="Browse Table"/></a>
                	
                	<a href="<%=request.getContextPath() %>/dba/createtbl.jsp?newtblname=<%=localtable %>&db=<%=db %>&alter=yes"> <img class="icon" src="<%=request.getContextPath() %>/dba/Images/edit.png" alt="Drop Table" /></a>
                	<a href="<%=request.getContextPath() %>/dba/drop.jsp?table=<%=localtable %>&db=<%=db %>&empty=yes"> <img class="icon" src="<%=request.getContextPath() %>/dba/Images/empty.png" alt="Drop Table" /></a>
                	<a href="<%=request.getContextPath() %>/dba/drop.jsp?table=<%=localtable %>&db=<%=db %>"> <img class="icon" src="<%=request.getContextPath() %>/dba/Images/delete.png" alt="Drop Table" /></a>
                	</center>
                    </td>
...
<form action="<%=request.getContextPath() %>/dba/createtbl.jsp?db=<%=db %>" method="post" onSubmit="return validate(this)">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  
    
     <img class="icon" src="<%=request.getContextPath() %>/dba/Images/arrow.png" alt="-" width="5" height="9"> Create New Table<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Name <input type="text" name="newtblname" maxlength=<%=dbmd.getMaxTableNameLength() %> >&nbsp; No. of Fields&nbsp; <input type="text" name="newtblfields" maxlength=<%=String.valueOf(dbmd.getMaxColumnsInTable()).length() %>> &nbsp; 
