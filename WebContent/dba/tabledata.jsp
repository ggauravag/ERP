...
<html>
  ...
  <body bgcolor="#f5f5f5">
           ...
           <tr>
           <td colspan="2">
           <a href="javascript:dRecord('<%=request.getContextPath() %>/dba/deletedata.jsp?db=<%=db%>&table=<%=table%>&field=<%=rsmd.getColumnName(1) %>&val=<%=rst.getString(1) %>')"><img class="icon" src="<%=request.getContextPath() %>/dba/Images/Delete.gif" alt="Delete Record"/></a>
           </td>           
         <% 
                for(int i=1;i<=rsmd.getColumnCount(); i++) {
            ...
            <input type="submit" value="Delete" name="delete"><br>
              <form method=post action="<%=request.getContextPath() %>/dba/insertdata.jsp?newtblfields=<%=rsmd.getColumnCount()%>&db=<%=db %>&table=<%=table %>">
           <br>
            &nbsp; &nbsp;Insert New Row
           <br>
           &nbsp; &nbsp; &nbsp;
